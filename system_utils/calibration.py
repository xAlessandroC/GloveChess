"""
    This module handles the camera calibration
"""
...
def processImage(fn, pattern_points):
    print('[CALIBRATION]: processing {}'.format(fn))
    img = cv2.imread(fn, cv2.IMREAD_GRAYSCALE)

    if img is None:
        print("[CALIBRATION]: Failed to load", fn)
        return None
    ...
    if not found:
        print('[CALIBRATION]: chessboard not found')
        return None
...
def calibration():
    ...
    print("[CALIBRATION]: Calibration completed: RMS =",rms)
...
def calibrate():
    ...
    if len(os.listdir(parameters_path)) != 4:
        print("[CALIBRATION]: Error, Parameters file not present ", len(os.listdir(parameters_path)))
        ...
        print("[CALIBRATION]: Loading calibration results...")
